...
- **Stage 4 — `build-employee-index.cob` + `payroll-indexed.cob`**: converts
  the employee master file to `ORGANIZATION IS INDEXED` keyed by employee
  ID, replaces `tax-calc.cob`'s hardcoded `IF`/`ELSE` tax brackets with an
  `OCCURS` table walked by `SEARCH`, and demonstrates both a random `READ`
  by key and a `START` + sequential `READ NEXT` pass over the same file.
...
# Stage 4 — build the indexed master file first (one-time, or whenever
# data/employees.dat changes); it's a generated binary artifact and is
# .gitignore'd, not checked in
cobc -x -I copybooks -o build-employee-index src/build-employee-index.cob
./build-employee-index

# then compile and run the Stage 4 driver, same two-source pattern as
# Stage 3 (tax-calc.cob's table-lookup version, see STAGE-NOTES.md)
cobc -x -I copybooks -o payroll-indexed src/payroll-indexed.cob src/tax-calc.cob
./payroll-indexed
```
...
(`payroll-v1`, `payroll-report`, `payroll-net`, `payroll-indexed`,
`build-employee-index`, `hello`) — they're build artifacts, not checked in.
Stage 4's indexed master file (`data/employees-indexed.dat`, a binary
Berkeley DB btree file) is likewise `.gitignore`d and regenerated by
`build-employee-index` rather than checked in.
...
`payroll-summary.out`, `payroll-net-report.out`, `payroll-indexed-report.out`)
as the current known-good baseline. Stage 4's tax-table refactor was
additionally verified by diffing `payroll-net`'s output before and after the
change — byte-identical, confirming the `OCCURS`/`SEARCH` lookup produces the
same tax amounts as Stage 3's hardcoded `IF`/`ELSE` for the same brackets.
